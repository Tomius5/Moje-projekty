        ...
        private Tuple<int, byte> GetBestBotMove()
        {
            ...
            int minimax(List<int> piles, int depth, bool maximizingPlayer)
            {
              ...
              //   if (depth == 0)
                ...
                // správně: konec hry je, když už nejsou žádné sirky
                if (piles.Sum() == 0)
                {
                ...
                if (maximizingPlayer)
                {
                    ...
                    {
                        for (byte take = 1; take <= piles[i]; take++)
                        {
                            // piles[i] -= take;
                            var newPiles = piles.ToList();
                            newPiles[i] -= take;
                            int score = minimax(newPiles, depth - 1, false);
                            // bestScore = score;
                ...
                else
                {
                    ...
                    for (int i = 0; i < piles.Count; i++)
                    {
                        for (byte take = 1; take <= piles[i]; take++)
                        {
                            ...
                            int score = minimax(newPiles, depth - 1, true);
